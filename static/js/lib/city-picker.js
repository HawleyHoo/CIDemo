...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            ...
            this.$textspan = $(textspan).insertAfter(this.$element);  //外面那个input span
            this.$dropdown = $(dropdown).insertAfter(this.$textspan); //弹出整个框
        ...
        bind: function () {
            ...
            this.$dropdown.on('click', '.city-select a', function () {
				var slef = this;
				$this.options.spanClick && $this.options.spanClick(this,function(){

                    var $select = $(slef).parents('.city-select');
					var $active = $select.find('a.active');
					var last = $select.next().length === 0;
					$active.removeClass('active');
					$(slef).addClass('active');
					if ($active.data('code') !== $(slef).data('code')) {
						$select.data('item', {
							address: $(slef).attr('title'), code: $(slef).data('code')
						});
						$(slef).trigger(EVENT_CHANGE);
						$this.feedText();
						$this.feedVal(true);
						if (last) {
							$this.close();
        ...
        output: function (type) {
			...
			// console.log(type)
			// console.log(ChineseDistricts[code])

			if(type=== PROVINCE){
            //    districts = PROVINCE_DATA
               districts = this.options.data.PROVINCE_DATA
            }else if(type=== CITY){
            //    districts = CITY_DATA
               districts = this.options.data.CITY_DATA

			}else if(type== DISTRICT){
				// districts = DISTRICT_DATA
               districts = this.options.data.DISTRICT_DATA
			}else{
				districts = null
			}
